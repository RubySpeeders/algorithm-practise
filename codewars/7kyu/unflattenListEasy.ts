...
//My Solution:
//Truly a brute force attempt, but it's just to get juices flowing

export function unflatten(flatArray: any[]): any[] {
  let copyArray = [...flatArray];
  const resultsArray: any = [];
  for (let i = 0; i < flatArray.length; ) {
    if (flatArray[i] < 3) {
      resultsArray.push(flatArray[i]);
      i++;
    } else {
      const subArray: any = [];
      subArray.push(...copyArray.splice(i, flatArray[i]));
